...
import { parseWithMarked, createMarkedWithCallouts } from './marked-callout'
import { parseWithMarkdownIt, createMarkdownItWithCallouts, findCalloutBlocks, parseCalloutHeader } from './mdit-callout-restructured'
...
function result(label: string, passed: boolean, detail?: string) {
...
interface TestCase {
  name: string
  input: string
  /** Validate the HTML output. Return error message or null if OK. */
  validate: (html: string, engine: string) => string | null
}
...
const TEST_CASES: TestCase[] = [
  // ── Basic callout ──
  {
    ...
    validate(html, engine) {
      if (!html.includes('callout')) return `Missing "callout" class`
      if (!html.includes('callout-note')) return `Missing "callout-note" class`
      if (!html.includes('data-callout="note"')) return `Missing data-callout attribute`
      if (!html.includes('callout-icon')) return `Missing callout icon`
      if (!html.includes('callout-title-text')) return `Missing title text container`
      if (!html.includes('This is a note')) return `Missing title content`
      if (!html.includes('With some content')) return `Missing body content`
      return null
    },
  },
  ...
  // ── Callout with no body ──
  {
    ...
    validate(html, engine) {
      if (!html.includes('callout-warning')) return `Missing callout-warning`
      if (!html.includes('Just a title')) return `Missing title`
      return null
    },
  },
  ...
  // ── All 14 types ──
  {
    ...
    validate(html, engine) {
      ...
      for (const t of types) {
        if (!html.includes(`callout-${t}`)) return `Missing callout-${t}`
      }
      return null
    },
  },
  ...
  // ── Type aliases ──
  {
    ...
    validate(html, engine) {
      if (!html.includes('callout-tip')) return `hint should resolve to tip`
      if (!html.includes('callout-danger')) return `error should resolve to danger`
      if (!html.includes('callout-abstract')) return `summary should resolve to abstract`
      return null
    },
  },
  ...
  // ── Foldable expanded ──
  {
    ...
    validate(html, engine) {
      if (!html.includes('data-callout-foldable')) return `Missing foldable attribute`
      if (!html.includes('data-callout-collapsed="false"') && !html.includes('data-callout-collapsed="false"')) {
        // Marked may render differently
        if (!html.includes('open') && !html.includes('collapsed')) return `Missing fold state`
      }
      if (!html.includes('<details')) return `Foldable should use <details>`
      if (!html.includes('<summary')) return `Foldable should use <summary>`
      if (!html.includes('callout-fold-icon')) return `Missing fold chevron`
      return null
    },
  },
  ...
  // ── Foldable collapsed ──
  {
    ...
    validate(html, engine) {
      if (!html.includes('data-callout-foldable')) return `Missing foldable attribute`
      if (!html.includes('<details')) return `Foldable should use <details>`
      if (!html.includes('\u25B8') && !html.includes('▸')) return `Missing collapsed chevron (▸)`
      return null
    },
  },
  ...
  // ── Multi-paragraph body ──
  {
    ...
    validate(html, engine) {
      if (!html.includes('First paragraph')) return `Missing first paragraph`
      if (!html.includes('Second paragraph')) return `Missing second paragraph`
      return null
    },
  },
  ...
  // ── Inline formatting in body ──
  {
    ...
    validate(html, engine) {
      if (!html.includes('<strong>bold</strong>')) return `Missing bold formatting`
      if (!html.includes('<em>italic</em>')) return `Missing italic formatting`
      return null
    },
  },
  ...
  // ── Regular blockquote unaffected ──
  {
    ...
    validate(html, engine) {
      if (html.includes('class="callout')) return `Regular blockquote should not have callout class`
      if (html.includes('data-callout')) return `Regular blockquote should not have data-callout attribute`
      if (!html.includes('<blockquote')) return `Should render as blockquote`
      return null
    },
  },
  ...
  // ── Mixed blockquotes and callouts ──
  {
    ...
    validate(html, engine) {
      ...
      if (calloutCount < 1) return `Should have at least one callout`
      return null
    },
  },
  ...
  // ── Auto-generated title ──
  {
    ...
    input: '> [!warning]',
    validate(html, engine) {
      if (!html.includes('Warning') && !html.includes('warning')) return `Should auto-generate title "Warning"`
      return null
    },
  },
  ...
  // ── Multiple callouts in sequence ──
  {
    ...
    input: '> [!note] First\n\n> [!warning] Second\n\n> [!danger] Third',
    validate(html, engine) {
      if (!html.includes('callout-note')) return `Missing first callout`
      if (!html.includes('callout-warning')) return `Missing second callout`
      if (!html.includes('callout-danger')) return `Missing third callout`
      return null
    },
  },
]
...
const EDGE_CASES: TestCase[] = [
  {
    ...
    validate(html, engine) {
      if (!html.includes('callout-my-custom-type')) return `Should support custom types`
      return null
    },
  },
  {
    ...
    validate(html, engine) {
      if (!html.includes('console.log')) return `Should preserve inline code`
      return null
    },
  },
  {
    ...
    input: '> [!tip]',
    validate(html, engine) {
      if (!html.includes('callout-tip')) return `Should still render callout`
      if (!html.includes('Tip')) return `Should auto-generate title`
      return null
    },
  },
  {
    ...
    validate(html, engine) {
      if (!html.includes('https://example.com')) return `Should preserve link`
      return null
    },
  },
  {
    ...
    validate(html, engine) {
      if (!html.includes('Line 1') || !html.includes('Line 50')) return `Should preserve all lines`
      return null
    },
  },
]
...
function benchmark(name: string, fn: () => void, iterations: number = 1000): { avg: number, min: number, max: number, total: number } {
  // Warm up
...
function main() {
  ...
  for (const tc of allTests) {
    // markdown-it
    try {
      const mditHtml = parseWithMarkdownIt(tc.input)
      const mditErr = tc.validate(mditHtml, 'markdown-it')
      if (mditErr) { result(`[md-it] ${tc.name}`, false, mditErr); mditFailed++ }
      else { result(`[md-it] ${tc.name}`, true); mditPassed++ }
    } catch (e: any) {
      result(`[md-it] ${tc.name}`, false, `EXCEPTION: ${e.message}`); mditFailed++
    }

    // Marked.js
    try {
      const markedHtml = parseWithMarked(tc.input)
      const markedErr = tc.validate(markedHtml, 'marked')
      if (markedErr) { result(`[marked] ${tc.name}`, false, markedErr); markedFailed++ }
      else { result(`[marked] ${tc.name}`, true); markedPassed++ }
    } catch (e: any) {
      result(`[marked] ${tc.name}`, false, `EXCEPTION: ${e.message}`); markedFailed++
    }
  }
  ...
  const mditHtml = parseWithMarkdownIt(basicInput)
  const markedHtml = parseWithMarked(basicInput)
  ...
  for (const bm of benchmarks) {
    ...
    const mditResult = benchmark('markdown-it', () => parseWithMarkdownIt(bm.input), bm.iters)
    const markedResult = benchmark('Marked.js', () => parseWithMarked(bm.input), bm.iters)
  ...
  const createMdit = benchmark('create md-it', () => createMarkdownItWithCallouts(), 500)
  const createMarked = benchmark('create marked', () => createMarkedWithCallouts(), 500)
