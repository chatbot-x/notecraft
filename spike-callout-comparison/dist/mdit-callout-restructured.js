...
exports.createMarkdownItWithCallouts = createMarkdownItWithCallouts;
exports.parseWithMarkdownIt = parseWithMarkdownIt;
exports.findCalloutBlocks = findCalloutBlocks;
exports.transformCalloutBlocks = transformCalloutBlocks;
exports.buildTitleTokens = buildTitleTokens;
exports.parseCalloutHeader = parseCalloutHeader;
 ...
 */
function findCalloutBlocks(tokens) {
    const results = [];
    for (let i = 0; i < tokens.length; i++) {
        ...
        for (let j = i + 1; j < closeIdx; j++) {
            if (tokens[j].type === 'paragraph_open' && paraOpenIdx === -1)
                paraOpenIdx = j;
            if (tokens[j].type === 'inline' && inlineIdx === -1)
                inlineIdx = j;
            if (tokens[j].type === 'paragraph_close' && paraCloseIdx === -1)
                paraCloseIdx = j;
        }
        if (inlineIdx === -1)
            continue;
 ...
 */
function transformCalloutBlocks(state, blocks) {
    ...
    for (let r = blocks.length - 1; r >= 0; r--) {
        ...
        const hasContent = inlineToken.children && inlineToken.children.some((c) => (c.type === 'text' && c.content.trim() !== '') || (c.type !== 'text' && c.type !== 'softbreak'));
        let removeCount = 0;
 ...
 */
function buildTitleTokens(state, parsed, meta) {
    const titleTokens = [];
    // Title wrapper
 ...
 */
function registerCalloutRenderers(md) {
    const customRules = [
 ...
 */
function createMarkdownItWithCallouts() {
    const md = new markdown_it_1.default({ html: false, linkify: true, typographer: true });
 ...
 */
function parseWithMarkdownIt(src) {
